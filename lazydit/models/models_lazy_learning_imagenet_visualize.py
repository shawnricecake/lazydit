...
from lazydit.models.models_timm_vision_transformer import PatchEmbed, Attention, Mlp
...
class DiTBlock(nn.Module):
    ...
    def forward(
        ...
        if not lazy and not accelerate:
            temp = self.attn(modulate(self.norm1(x), shift_msa, scale_msa))
            if sample_cache:
                self.cache_attn = temp
            x = x + gate_msa.unsqueeze(1) * temp

            temp = self.mlp(modulate(self.norm2(x), shift_mlp, scale_mlp))
            if sample_cache:
                self.cache_mlp = temp
            x = x + gate_mlp.unsqueeze(1) * temp
            return x

        # xuan: acceleration here
        if accelerate:
            count_attn = 0
            count_mlp = 0
            ...
            if self.cache_attn is None:
                ...
                if torch.all(mask_attn):  # xuan: all need to stay, can not jump
                    ...
                    if torch.all(mask_attn == 0):       # jump all attn
                        ...
                        final[mask_attn] = self.attn(temp[mask_attn])
        ...
        if self.cache_attn is not None:
            ...
            temp = self.attn(temp) * mask + self.cache_attn * (torch.ones_like(mask) - mask)
            x = x + gate_msa.unsqueeze(1) * temp
            self.cache_attn = temp
        else:
            temp = self.attn(temp)
        ...
        if self.cache_mlp is not None:
            ...
            temp = self.mlp(temp) * mask + self.cache_mlp * (torch.ones_like(mask) - mask)
            x = x + gate_mlp.unsqueeze(1) * temp
            self.cache_mlp = temp
        else:
            temp = self.mlp(temp)
...
class DiT(nn.Module):
    ...
    def forward(self, x, t, y, label_for_dropout=None):
        ...
        for block in self.blocks:
            if self.accelerate:
                x, count_attn, count_mlp, mask_attn, mask_mlp = block(x, c, accelerate=True, timestep=timestep)  # (N, T, D)
                self.jump_attn += count_attn
                self.jump_mlp += count_mlp
                mask_attn_total.append(mask_attn)
                mask_mlp_total.append(mask_mlp)
            elif not self.lazy:
                x = block(x, c, sample_cache=self.sample_cache)  # (N, T, D)
            elif self.lazy:
                x, mask_attn, mask_mlp = block(x, c, lazy=True, timestep=timestep)  # (N, T, D)
                mask_attn_total.append(mask_attn)
                mask_mlp_total.append(mask_mlp)
        ...
        if self.lazy or self.accelerate:
